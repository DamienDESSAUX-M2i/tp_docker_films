...
def load_csv(file_path: Path) -> dict[int, Movie]:
    ...
    with open(file=file_path, mode="rt", encoding="utf-8", newline="") as csv_file:
        ...
        for row in csv_reader:
            movie: Movie = Movie(
                titre=row["titre"],
                annee_production=int(row["annee_production"]),
                genre=Genre(row["genre"]),
                age_limite=int(row["age_limite"]),
            )
...
def write_csv(file_path: Path, movies: dict[int, Movie]) -> None:
    if not file_path.exists():
        raise PathDoesNotExistException(f"path : '{file_path}' does not exist.")
    if not (
        isinstance(movies, dict)
        and all(isinstance(movie, Movie) for movie in movies.values())
    ):
        raise TypeError(
            "Type of 'movies' must be dict whose values must be instances of class Movie."
        )
