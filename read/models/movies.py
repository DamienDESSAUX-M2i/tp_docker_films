...
class Movie:
    ...
    def __init__(
        self, titre: str, annee_production: int, genre: Genre, age_limite: int
    ) -> None:
    ...
    def __str__(self) -> None:
        ...
        infos: list[str] = ["=== Infos Film ==="]
        infos.append(f"Titre : {self._titre}")
        infos.append(f"Année de production : {self._annee_production}")
        infos.append(f"Genre : {self._genre.value}")
        infos.append(f"Age limite : {self._age_limite}")
        return "\n".join(infos)
